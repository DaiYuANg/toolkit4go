...
func atlasFallbackFromBasicGoType(rawType string, typ reflect.Type) atlasschema.Type {
	...
	if kind == reflect.Int64 || isSignedIntKind(kind) {
		return &atlasschema.IntegerType{T: rawType}
	}
	if kind == reflect.Uint64 || isUnsignedIntKind(kind) {
...
func atlasColumnsByName(table *atlasschema.Table, names []string) []*atlasschema.Column {
	columns := make([]*atlasschema.Column, 0, len(names))
	for i := range names {
		column, ok := table.Column(names[i])
		if ok {
